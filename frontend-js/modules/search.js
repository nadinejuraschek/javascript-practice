...
export default class Search {
  ...
  openOverlay() {
    ...
    // set cursor to input field on opening the modal
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      ${posts.map(post => {
        ...
        <span class="text-muted small">by ${post.author.username} on ${postDate.getMonth()}/${postDate.getDay()}/${postDate.getFullYear()}</span>
      </a>`;
      }).join("")}
    </div>`;
    } else {
      this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, we could not find any matching results.</p>`;
    };
    this.hideLoaderIcon();
